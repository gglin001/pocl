...
static cl_int PrintPlatformInfoSummary(cl_platform_id platformId) {
  ...
  errorCode |= AllocateAndGetPlatformInfoString(platformId, CL_PLATFORM_NAME,
                                                &platformName);
  errorCode |= AllocateAndGetPlatformInfoString(platformId, CL_PLATFORM_VENDOR,
                                                &platformVendor);
  errorCode |= AllocateAndGetPlatformInfoString(platformId, CL_PLATFORM_VERSION,
                                                &platformVersion);
...
static cl_int PrintDeviceInfoSummary(cl_device_id *devices,
                                     cl_uint numDevices) {
  cl_int errorCode = CL_SUCCESS;
  ...
  for (i = 0; i < numDevices; i++) {
    errorCode |= clGetDeviceInfo(devices[i], CL_DEVICE_TYPE, sizeof(deviceType),
                                 &deviceType, NULL);
    errorCode |=
        AllocateAndGetDeviceInfoString(devices[i], CL_DEVICE_NAME, &deviceName);
    errorCode |= AllocateAndGetDeviceInfoString(devices[i], CL_DEVICE_VENDOR,
                                                &deviceVendor);
    errorCode |= AllocateAndGetDeviceInfoString(devices[i], CL_DEVICE_VERSION,
                                                &deviceVersion);
    errorCode |= AllocateAndGetDeviceInfoString(devices[i], CL_DEVICE_PROFILE,
                                                &deviceProfile);
    errorCode |= AllocateAndGetDeviceInfoString(devices[i], CL_DRIVER_VERSION,
                                                &driverVersion);
